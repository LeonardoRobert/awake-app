...
create policy "profile_ministerios_ocultar_teste" on public.profile_ministerios
  as restrictive for select
  using (
    not coalesce((select eh_conta_teste from public.profiles where id = profile_ministerios.profile_id), false)
    or is_conta_teste()
  );
...
create policy "contribuicoes_ocultar_teste" on public.contribuicoes
  as restrictive for select
  using (
    not coalesce((select eh_conta_teste from public.profiles where id = contribuicoes.profile_id), false)
    or is_conta_teste()
  );
...
create policy "escalas_servico_ocultar_teste" on public.escalas_servico
  as restrictive for select
  using (
    not coalesce((select eh_conta_teste from public.profiles where id = escalas_servico.criado_por), false)
    or is_conta_teste()
  );
  ...
  using (
    not exists (
      ...
      where es.id = escala_servico_posicoes.escala_id
        and coalesce((select eh_conta_teste from public.profiles where id = es.criado_por), false)
    )
    or is_conta_teste()
  );
...
create policy "eventos_ocultar_teste" on public.eventos
  as restrictive for select
  using (
    not coalesce((select eh_conta_teste from public.profiles where id = eventos.criado_por), false)
    or is_conta_teste()
  );
...
create policy "pedidos_oracao_ocultar_teste" on public.pedidos_oracao
  as restrictive for select
  using (
    not coalesce((select eh_conta_teste from public.profiles where id = pedidos_oracao.profile_id), false)
    or is_conta_teste()
  );
...
create policy "testemunhos_ocultar_teste" on public.testemunhos
  as restrictive for select
  using (
    not coalesce((select eh_conta_teste from public.profiles where id = testemunhos.profile_id), false)
    or is_conta_teste()
  );
...
create policy "visitantes_pv_ocultar_teste" on public.visitantes_primeira_vez
  as restrictive for select
  using (
    not coalesce((select eh_conta_teste from public.profiles where id = visitantes_primeira_vez.registrado_por), false)
    or is_conta_teste()
  );
